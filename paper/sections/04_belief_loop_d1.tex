...
\label{lem:high-type-monotone-withdrawal}
...
\item off-path beliefs satisfy D1, with the strict-response richness condition of Lemma~\ref{lem:d1-off-path-reversal};
...
Third, evaluate \(\mathcal C_0\). Under \(\mathcal C_0\), \(s_A\) is off path. Since \(q>q_{\mathrm{D1}}\), Lemma~\ref{lem:d1-off-path-reversal} and its general-price restatement imply
\[
\mathcal D_H(q)\subsetneq \mathcal D_L(q),
\]
