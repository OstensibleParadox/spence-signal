...
\]
where $\Phi_\theta>0$ is the true-type exposure cost; Section~\ref{sec:psychological-exposure} gives one primitive foundation for the reversed ranking \(\Phi_H>\Phi_L\). The parameter $\beta\in(0,1)$ is the discount factor.
...
\label{ass:static-benchmark}
\[
B_H>0,\qquad B_L<0.
\]
\end{assumption}

Under Assumption~\ref{ass:static-benchmark}, with no anticipated repricing $s_A$ is uniquely attractive only to the high type.

\subsection{Anticipatory reversal object}

The model's distinctive case is high-type exposure vulnerability: the same type that gains from ambitious separation can face the larger future exposure cost. Assumption~\ref{ass:static-benchmark} therefore identifies the low-scrutiny rent before future exposure is priced; it does not guarantee that the high type keeps using \(s_A\) when the transition probability into high scrutiny is large. Section~\ref{sec:belief-loop} defines the IC and D1 thresholds and shows how their maximum selects safe pooling.
