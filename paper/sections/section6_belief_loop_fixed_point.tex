...
Section~\ref{sec:polarization} gives the observable shadow of withdrawal. Once scrutiny risk is high enough, the ambitious signal need not disappear; it can survive as a polarized object, carried by extreme high types and by agents with weak anticipatory discipline after ordinary high types have exited. That result is cross-sectional. It says what the ambitious signal looks like after withdrawal has started, but it does not yet show why withdrawal is a self-confirming equilibrium object.

The missing step is logically small but not vacuous. If high types are expected to remain on \(s_A\), the signal carries a high-type premium. If high types are expected to withdraw, \(s_A\) becomes off path, and D1 can remove that premium. Thus the sender's participation decision and the receiver's interpretation of the signal must be solved together.

The local forces are already in place. Adverse repricing is pinned to true type, \(\Phi_H>\Phi_L\) (Section~\ref{sec:surprise}). When the transition probability into high scrutiny is sufficiently large, D1 no longer assigns an off-path ambitious signal to the high type (Section~\ref{sec:dynamic}). This section turns those local comparisons into a fixed-point selection result.
